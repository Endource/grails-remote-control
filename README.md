...
**Version 3.0 of this plugin requires Grails >= 3.0.0 and will not work on earlier versions.**
...
```sh
grails test run-app -port=8081
grails test-app -integration -DbaseUrl=http://localhost:8081
```
or...

```sh
grails war
java -Dgrails.env=test -jar build/libs/mywar-0.1.war
grails test-app -integration -DbaseUrl=http://localhost:8081
```

This is going to fail because the `Person` related code will not execute in the JVM of the running grails application.
